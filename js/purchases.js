...
(function () {
    const baseUrl = ''; // use relative root; adjust if your API is elsewhere.

    // small helpers fallback
    const UtilsLocal = {
        showNotification: (msg, type = 'info') => { if (window.Utils && Utils.showNotification) return Utils.showNotification(msg, type); alert(msg); },
        formatCurrency: (v) => {
            if (window.Utils && Utils.formatCurrency) return Utils.formatCurrency(v);
            return (Number(v) || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' درهم';
        },
        formatDate: (iso) => {
    ...
    const ApiClient = {
        async productsList() {
            if (window.API && API.Products && API.Products.list) return API.Products.list();
            const r = await fetch(baseUrl + '/api/Products', { headers: { 'accept': 'application/json' } });
            return await r.json();
        },
        async productGet(id) {
            if (window.API && API.Products && API.Products.get) return API.Products.get(id);
            const r = await fetch(baseUrl + `/api/Products/${id}`, { headers: { 'accept': 'application/json' } });
            return await r.json();
        },
        async purchasesList() {
            if (window.API && API.Purchases && API.Purchases.list) return API.Purchases.list();
            const r = await fetch(baseUrl + '/api/Purchases', { headers: { 'accept': 'application/json' } });
            return await r.json();
        },
        async purchasesCreate(payload) {
            if (window.API && API.Purchases && API.Purchases.create) return API.Purchases.create(payload);
        ...
        async purchasesDelete(id) {
            if (window.API && API.Purchases && API.Purchases.remove) return API.Purchases.remove(id);
            const r = await fetch(baseUrl + `/api/Purchases/${id}`, { method: 'DELETE' });
            return await r.json();
        },
        async purchasesUpdateStatus(id, payload) {
            // prefer PUT /api/Purchases/{id}/status or PATCH depending on backend; try PUT first
            if (window.API && API.Purchases && API.Purchases.updateStatus) return API.Purchases.updateStatus(id, payload);
        ...
        async inventoryUpdateStock(payload) {
            if (window.API && API.Inventory && API.Inventory.updateStock) return API.Inventory.updateStock(payload);
    ...
    async function loadProducts() {
        try {
            ...
            const arr = (res && res.data) ? res.data : (Array.isArray(res) ? res : (res?.data ?? []));
            state.products = arr;
        } catch (err) {
    ...
    function renderPurchases() {
        ...
        if (s) {
            list = list.filter(p => (p.purchase_number || '').toLowerCase().includes(s) || (p.supplier?.name || '').toLowerCase().includes(s));
        }
        if (state.filters.dateFrom) {
            list = list.filter(p => new Date(p.created_at) >= new Date(state.filters.dateFrom));
        }
        if (state.filters.dateTo) {
            list = list.filter(p => new Date(p.created_at) <= new Date(state.filters.dateTo));
        }
    ...
    function createPurchaseRow(purchase) {
        const paymentStatusColor = purchase.payment_status === 'PAID' ? 'bg-green-100 text-green-800' :
            purchase.payment_status === 'PARTIAL' ? 'bg-yellow-100 text-yellow-800' :
            'bg-red-100 text-red-800';

        const deliveryStatusColor = purchase.delivery_status === 'DELIVERED' ? 'bg-green-100 text-green-800' :
            purchase.delivery_status === 'PARTIAL' ? 'bg-yellow-100 text-yellow-800' :
            'bg-red-100 text-red-800';

        return `
            <tr class="purchase-row">
                <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm font-medium text-slate-900">${purchase.purchase_number ?? purchase.id}</div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm text-slate-900">${purchase.supplier?.name || 'مورد'}</div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm text-slate-600">${UtilsLocal.formatDate(purchase.created_at)}</div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-center">
                    <div class="text-sm text-slate-900">${(purchase.items || []).length}</div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-right">
                    <div class="text-sm font-semibold text-slate-900">${UtilsLocal.formatCurrency(purchase.total)}</div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-center">
                    <span class="px-2 py-1 text-xs font-medium rounded-full ${paymentStatusColor}">
                        ${purchase.payment_status === 'PAID' ? 'مدفوع' : purchase.payment_status === 'PARTIAL' ? 'مدفوع جزئياً' : 'قيد الانتظار'}
                    </span>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-center">
                    <span class="px-2 py-1 text-xs font-medium rounded-full ${deliveryStatusColor}">
                        ${purchase.delivery_status === 'DELIVERED' ? 'تم التوصيل' : purchase.delivery_status === 'PARTIAL' ? 'تم التوصيل جزئياً' : 'قيد الانتظار'}
                    </span>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-right">
                    <div class="flex space-x-2">
                        <button onclick="Purchases.viewPurchase('${purchase.id}')" class="text-blue-600 hover:text-blue-800 text-sm">
                            <i class="fas fa-eye"></i>
                        </button>
                        <button onclick="Purchases.updatePurchaseStatusPrompt('${purchase.id}')" class="text-green-600 hover:text-green-800 text-sm">
                            <i class="fas fa-edit"></i>
                        </button>
                        <button onclick="Purchases.deletePurchase('${purchase.id}')" class="text-red-600 hover:text-red-800 text-sm">
    ...
    function updateStats() {
        ...
        const todayPurchases = state.purchases.filter(p => (p.created_at || '').split('T')[0] === today);
        const todayTotal = todayPurchases.reduce((s,p) => s + (p.total || 0), 0);
        const totalPurchases = state.purchases.reduce((s,p) => s + (p.total || 0), 0);
    ...
    function filterProductList(searchTerm) {
        ...
        const products = state.products.filter(product => {
            ...
            return (product.variants || []).some(v => (v.specification || '').toLowerCase().includes(term) || (v.sku || '').toLowerCase().includes(term));
        });
        const list = $('productList');
        list.innerHTML = products.map(p => (p.variants || []).map(v => `
            <div class="p-3 border-b border-slate-200 hover:bg-slate-50 cursor-pointer" 
    ...
    function addPurchaseItem(productId, variantId) {
        ...
        const existing = state.purchaseItems.find(it => it.product_id === productId && it.variant_id === variantId);
        if (existing) {
    ...
    function renderPurchaseItems() {
        ...
        container.innerHTML = (state.purchaseItems || []).map((item, idx) => `
            <div class="purchase-item p-4 border border-slate-200 rounded-lg">
                <div class="grid grid-cols-1 md:grid-cols-5 gap-4 items-center">
                    <div class="md:col-span-2">
                        ...
                        <div class="text-sm text-slate-600">${item.variant_name}</div>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-slate-700 mb-1">الكمية</label>
                        <input type="number" min="1" value="${item.quantity}" onchange="Purchases.updatePurchaseItemQuantity(${idx}, this.value)"
                    ...
                    <div>
                        ...
                        <input type="number" min="0" step="0.01" value="${item.unit_cost}" onchange="Purchases.updatePurchaseItemCost(${idx}, this.value)"
                               class="w-full px-2 py-1 text-sm border border-slate-300 rounded focus:ring-1 focus:ring-blue-500">
                    </div>
                    <div class="flex items-center justify-between">
                        <div>
                            <div class="text-sm font-semibold text-slate-900">${UtilsLocal.formatCurrency(item.total_cost)}</div>
                        </div>
    ...
    function updatePurchaseItemQuantity(index, qty) {
        ...
        state.purchaseItems[index].total_cost = state.purchaseItems[index].unit_cost * qty;
        renderPurchaseItems();
        updatePurchaseSummary();
    }
    ...
    function updatePurchaseItemCost(index, cost) {
        ...
        state.purchaseItems[index].total_cost = state.purchaseItems[index].quantity * cost;
        renderPurchaseItems();
        updatePurchaseSummary();
    }
    ...
    function updatePurchaseSummary() {
        const subtotal = state.purchaseItems.reduce((s,i)=> s + (i.total_cost || 0), 0);
    ...
    async function savePurchase(e) {
        ...
        if (!supplierName) { UtilsLocal.showNotification('يرجى إدخال اسم المورد', 'error'); return; }
        ...
        try {
            ...
            const created = (createRes && createRes.data) ? createRes.data : (Array.isArray(createRes) ? createRes[0] : createRes);
            ...
            for (const it of payload.items) {
                try {
                    ...
                    UtilsLocal.showNotification(`فشل تحديث المخزون للعنصر: ${it.variantId}`, 'error');
                }
            }

            UtilsLocal.showNotification('تم إنشاء طلب الشراء وتحديث المخزون', 'success');
            await refreshAll();
            closePurchaseModal();
        } catch (err) {
    ...
    async function updatePurchaseStatusPrompt(id) {
        ...
        if (!purchase) return;
        const newPayment = prompt(`حالة الدفع الحالية: ${purchase.payment_status}\nأدخل الحالة الجديدة (PAID, PENDING, PARTIAL):`, purchase.payment_status) || '';
        const newDelivery = prompt(`حالة التوصيل الحالية: ${purchase.delivery_status}\nأدخل الحالة الجديدة (DELIVERED, PENDING, PARTIAL):`, purchase.delivery_status) || '';
            ...
            <div class="bg-white p-6 rounded-lg shadow-lg">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <div class="text-left">
                        ...
                        <p class="text-slate-600">رقم الطلب: ${purchase.purchase_number ?? purchase.id}</p>
                        <p class="text-slate-600">التاريخ: ${UtilsLocal.formatDate(purchase.created_at)}</p>
                    </div>
                </div>

                <div class="border-t border-b border-slate-200 py-4 mb-6">
                    <h4 class="font-semibold text-slate-900 mb-2">معلومات المورد:</h4>
                    <p class="text-slate-700">الاسم: ${purchase.supplier?.name || 'مورد'}</p>
                    ${purchase.supplier?.contact ? `<p class="text-slate-700">جهة الاتصال: ${purchase.supplier.contact}</p>` : ''}
                    ${purchase.supplier?.phone ? `<p class="text-slate-700">الهاتف: ${purchase.supplier.phone}</p>` : ''}
                    ${purchase.supplier?.address ? `<p class="text-slate-700">العنوان: ${purchase.supplier.address}</p>` : ''}
                </div>
                        ...
                        ${(purchase.items || []).map(item => {
                            ...
                            return `
                                <tr class="border-b border-slate-100">
                                    <td class="py-2 text-slate-700">${product.name_ar || ''} - ${item.variant_name || ''}</td>
                                    <td class="py-2 text-center text-slate-700">${item.quantity}</td>
                                    <td class="py-2 text-right text-slate-700">${UtilsLocal.formatCurrency(item.unit_cost)}</td>
                                    <td class="py-2 text-right text-slate-700">${UtilsLocal.formatCurrency(item.total_cost || (item.unit_cost*item.quantity))}</td>
                                </tr>
                            `;
                        }).join('')}
                    </tbody>
                ...
                <div class="flex justify-end">
                    <div class="w-64">
                        <div class="flex justify-between mb-2">
                            ...
                            <span class="font-semibold text-slate-900">${UtilsLocal.formatCurrency(purchase.subtotal)}</span>
                        </div>
                        <div class="flex justify-between mb-2">
                            <span class="font-medium text-slate-700">الضريبة:</span>
                            <span class="font-semibold text-slate-900">${UtilsLocal.formatCurrency(purchase.tax)}</span>
                        </div>
                        <div class="flex justify-between pt-2 border-t border-slate-200">
                            <span class="text-lg font-bold text-slate-900">الإجمالي:</span>
                            <span class="text-lg font-bold text-slate-900">${UtilsLocal.formatCurrency(purchase.total)}</span>
                        </div>
                    </div>
                </div>

                <div class="mt-6 pt-4 border-t border-slate-200">
                    <p class="text-sm text-slate-600">حالة الدفع: ${purchase.payment_status}</p>
                    <p class="text-sm text-slate-600">حالة التوصيل: ${purchase.delivery_status}</p>
                </div>
            </div>
        `;
    ...
    function exportPurchases() {
        ...
        if (!purchases.length) { UtilsLocal.showNotification('لا توجد مشتريات للتصدير', 'error'); return; }
    ...
    function setupEvents() {
        $('searchInput').addEventListener('input', (e)=> { state.filters.search = e.target.value; renderPurchases(); });
        $('dateFrom').addEventListener('change', (e)=> { state.filters.dateFrom = e.target.value; renderPurchases(); });
        $('dateTo').addEventListener('change', (e)=> { state.filters.dateTo = e.target.value; renderPurchases(); });

        $('productSearch').addEventListener('input', (e)=> filterProductList(e.target.value));

        $('purchaseForm').addEventListener('submit', savePurchase);

        // product selector open/close handlers
        document.querySelectorAll('[onclick="openProductSelector()"]').forEach(btn => btn.addEventListener('click', openProductSelector));
        document.querySelectorAll('[onclick="closeProductSelector()"]').forEach(btn => btn.addEventListener('click', closeProductSelector));
        // close purchase modal binding (top right 'x' button in modal already inline onclick)
        document.querySelectorAll('[onclick="closePurchaseModal()"]').forEach(btn => btn.addEventListener('click', closePurchaseModal));
        document.getElementById('logoutBtn')?.addEventListener('click', ()=> { if (window.Auth && Auth.logout) Auth.logout(); });
    }
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        if (window.Utils && Utils.setActiveNav) Utils.setActiveNav('purchases.html');
